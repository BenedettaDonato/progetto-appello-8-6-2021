...
public class PubblicatoreImpl extends UnicastRemoteObject implements Pubblicatore {
    ...
    private HashMap<FruitoreNotizie, HashSet<TipoRivista>> listaFruitori = new HashMap<>();     //Abbiamo scelto di utilizzare un HashSet perche non accetta duplicati rispetto ad un arrayList che invece li accetta.
                                                                                                //L'ordine in cui si estraggono gli elementi e CASUALE come per l'HashMap; poco importa, dato che l'ordine non ci interessa.

    private ProduttoreNotizie arrayProduttoreNotizie[];


    public PubblicatoreImpl(int numeroProduttoriNotizie) throws RemoteException {
        super();
    ...
    public synchronized void trasmettiEditoriali() throws RemoteException {
        for(Editoriale editoriale : listaEditoriali){
            ...
            for (Map.Entry<FruitoreNotizie, HashSet<TipoRivista>> entry : listaFruitori.entrySet()) {
                ...
                if(interessiFruitore.contains(editoriale.getTipoRivista())){    //Verifichiamo se il fruitore è interessato all'editoriale
                    try{
    ...
    public synchronized void aggiungiInteresseFruitore(FruitoreNotizie fruitoreNotizie, TipoRivista tipoRivista) throws RemoteException {
        //Caso 0: Non esiste l'HashSet collegata al fruitore di notizie considerato  (Non è presente la chiave nell'HashMap).
        if(!listaFruitori.containsKey(fruitoreNotizie)) {
            ...
            listaFruitori.get(fruitoreNotizie).add(tipoRivista);    //Non dobbiamo fare il controllo se il fruitore dice di essere interessato più di una volta ad un stesso TipoRivista,
                                                                    //proprio perchè l'HashSet non consente di inserire due valori uguali.
        }
    ...
    public synchronized void rimuoviInteresseFruitore(FruitoreNotizie fruitoreNotizie, TipoRivista tipoRivista) throws RemoteException {
        //Caso 0: Verifico se il fruitore è iscritto ad almeno un TipoRivista
        if(listaFruitori.containsKey(fruitoreNotizie)){
            ...
            if(!interesseRimosso){
                ...
                //Caso 3: Controlliamo se il fruitore è iscritto ad altri interessi. In caso contrario lo rimuoviamo.
    ...
    public static void main(String[] args) throws InterruptedException, RemoteException {
        ...
        Registry registry = LocateRegistry.createRegistry(1099);    //da controllare
